...
--만료되서 KH계정 비번을 KH123으로 바꾸고 NEW ORACLE 설정에서 연결해줌. / 만료되지 않도록 설정해줌.
...
MATERIALNAME VARCHAR2(100) PRIMARY KEY,
FAT NUMBER,
PROTEIN NUMBER,
CARBOHYDRATE NUMBER,
CALORIE NUMBER
...
BOARDNO NUMBER NOT NULL,
CONSTRAINT FK_RECEPIE_MATERIAL FOREIGN KEY(MATERIALNAME)REFERENCES MATERIAL(MATERIALNAME),
CONSTRAINT FK_RECEPIE_BOARD FOREIGN KEY(BOARDNO)REFERENCES BOARD(BOARDNO)
);


--맛집
--게시판번호(외래키), 이름, 주소, 위도, 경도
CREATE TABLE STORE(
BOARDNO NUMBER NOT NULL,
STORE_NAME VARCHAR2(1000) NOT NULL,
STORE_ADDR VARCHAR2(2000) NOT NULL,
LATITUDE NUMBER,
LONGITUDE NUMBER,
CONSTRAINT FK_STORE_BOARD FOREIGN KEY(BOARDNO)REFERENCES BOARD(BOARDNO)
);
